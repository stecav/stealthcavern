...
void BehaviorStandard::doUpkeep() {
	...
	if (e->stats.hull > 0 || e->stats.effects.triggered_death) powers->activatePassives(&e->stats);
	e->stats.logic();
	if (e->stats.waypoint_pause_ticks > 0)
		e->stats.waypoint_pause_ticks--;

	// check for revive
	if (e->stats.hull <= 0 && e->stats.effects.revive) {
		e->stats.hull = e->stats.get(STAT_HULL_MAX);
		e->stats.alive = true;
	...
	if (e->stats.hull <= 0 && !(e->stats.cur_state == ENEMY_DEAD || e->stats.cur_state == ENEMY_CRITDEAD)) {
...
void BehaviorStandard::checkPower() {
	...
	if (los && (e->stats.cur_state == ENEMY_STANCE || e->stats.cur_state == ENEMY_MOVE)) {
		...
		if (e->stats.half_dead_power && e->stats.hull <= e->stats.get(STAT_HULL_MAX)/2) {
