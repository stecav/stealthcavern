...
PlatformOptions_t PlatformOptions = {true, false, CONFIG_MENU_TYPE_BASE, ""};
...
void PlatformSetPaths() {
	...
	// $XDG_CONFIG_HOME/stealthcavern/
	if (getenv("XDG_CONFIG_HOME") != NULL) {
		PATH_CONF = (std::string)getenv("XDG_CONFIG_HOME") + "/stealthcavern/";
	}
	// $HOME/.config/stealthcavern/
	else if (getenv("HOME") != NULL) {
		PATH_CONF = (std::string)getenv("HOME") + "/.config/";
		createDir(PATH_CONF);
		PATH_CONF += "stealthcavern/";
	}
	// ./config/
	else {
		PATH_CONF = "./config/";
	...
	// $XDG_DATA_HOME/stealthcavern/
	if (getenv("XDG_DATA_HOME") != NULL) {
		PATH_USER = (std::string)getenv("XDG_DATA_HOME") + "/stealthcavern/";
	}
	// $HOME/.local/share/stealthcavern/
	else if (getenv("HOME") != NULL) {
		PATH_USER = (std::string)getenv("HOME") + "/.local/";
		createDir(PATH_USER);
		PATH_USER += "share/";
		createDir(PATH_USER);
		PATH_USER += "stealthcavern/";
	}
	// ./saves/
	else {
		PATH_USER = "./userdata/";
	...
	if (getenv("XDG_DATA_DIRS") != NULL) {
		...
		while (pathtest != "") {
			if (!path_data) {
				PATH_DATA = pathtest + "/stealthcavern/";
	...
	// check /usr/local/share/stealthcavern/ and /usr/share/stealthcavern/ next
	if (!path_data) PATH_DATA = "/usr/local/share/stealthcavern/";
	if (!path_data && dirExists(PATH_DATA)) path_data = true;

	if (!path_data) PATH_DATA = "/usr/share/stealthcavern/";
	if (!path_data && dirExists(PATH_DATA)) path_data = true;

	// check "games" variants of these
	if (!path_data) PATH_DATA = "/usr/local/share/games/stealthcavern/";
	if (!path_data && dirExists(PATH_DATA)) path_data = true;

	if (!path_data) PATH_DATA = "/usr/share/games/stealthcavern/";
	if (!path_data && dirExists(PATH_DATA)) path_data = true;
