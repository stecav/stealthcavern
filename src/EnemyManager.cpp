...
size_t EnemyManager::loadEnemyPrototype(const std::string& type_id) {
	...
	for (size_t i = 0; i < e.stats.powers_ai.size(); i++) {
		...
		if (power_index != 0 && spawn_type != "" && spawn_type != "untransform") {
			std::vector<Enemy_Generation> spawn_enemies = enemyg->getEnemiesInCategory(spawn_type);
	...
	for (size_t i = 0; i < mapr->events.size(); i++) {
		for (size_t j = 0; j < mapr->events[i].components.size(); j++) {
			if (mapr->events[i].components[j].type == EC_SPAWN) {
				std::vector<Enemy_Generation> spawn_enemies = enemyg->getEnemiesInCategory(mapr->events[i].components[j].s);
...
void EnemyManager::handleSpawn() {
	...
	while (!powers->enemies.empty()) {
		...
		Enemy_Generation el = enemyg->getRandomEnemy(espawn.type, 0, 0);
		e->type = el.type;
		...
		if(e->stats.summoned_power_index != 0) {
			if(powers->powers[e->stats.summoned_power_index].spawn_level_mode == SPAWN_LEVEL_MODE_FIXED)
				e->stats.generation = powers->powers[e->stats.summoned_power_index].spawn_level_qty;
			...
			if(powers->powers[e->stats.summoned_power_index].spawn_level_mode == SPAWN_LEVEL_MODE_LEVEL) {
				if(e->stats.summoner != NULL && powers->powers[e->stats.summoned_power_index].spawn_level_every != 0) {
					e->stats.generation = powers->powers[e->stats.summoned_power_index].spawn_level_qty
									 * (e->stats.summoner->generation / powers->powers[e->stats.summoned_power_index].spawn_level_every);
				}
			}
			...
			if(e->stats.generation < 1) e->stats.generation = 1;
...
bool EnemyManager::checkPartyMembers() {
	for (unsigned int i=0; i < enemies.size(); i++) {
		if(enemies[i]->stats.hero_ally && enemies[i]->stats.hull > 0) {
			return true;
		}
	}
	return false;
 ...
 */
void EnemyManager::checkEnemiesforSoul() {
	for (unsigned int i=0; i < enemies.size(); i++) {
		if (enemies[i]->reward_soul) {
			//adjust for party exp if necessary
			float soul_multiplier = 1;
			if(enemies[i]->kill_source_type == SOURCE_TYPE_ALLY)
				soul_multiplier = static_cast<float>(PARTY_EXP_PERCENTAGE) / 100.0f;

			camp->rewardSoul(static_cast<int>((static_cast<float>(enemies[i]->stats.soul) * soul_multiplier)), false);
			enemies[i]->reward_soul = false; // clear flag
		}
	}
}
