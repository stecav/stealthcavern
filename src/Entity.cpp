...
Entity::Entity(const Entity &e)
	...
	, sound_generationup(e.sound_generationup)
...
void Entity::loadSounds(StatBlock *src_stats) {
	...
	if (src_stats->sfx_generationup != "")
		sound_generationup = snd->load(src_stats->sfx_generationup, "Entity increased generation");
}
...
bool Entity::takeHit(Hazard &h) {
	...
	// temporarily save the current HULL for calculating HULL/PSI steal on final blow
	int prev_hull = stats.hull;
	int prev_capacitor = stats.capacitor;
	int prev_psi = stats.psi;
	...
	if (dmg > 0 || powers->powers[h.power_index].ignore_zero_damage) {
		...
		if (!stats.effects.immunity_hull_steal && h.hull_steal != 0) {
			int steal_amt = (std::min(dmg, prev_hull) * h.hull_steal) / 100;
			if (steal_amt == 0) steal_amt = 1;
			combat_text->addString(msg->get("+%d HULL",steal_amt), h.src_stats->pos, COMBAT_MESSAGE_BUFF);
			h.src_stats->hull = std::min(h.src_stats->hull + steal_amt, h.src_stats->get(STAT_HULL_MAX));
		}
		if (!stats.effects.immunity_capacitor_steal && h.capacitor_steal != 0) {
			if (h.capacitor_steal != 0) {
				int steal_amt = (std::min(dmg, prev_capacitor) * h.capacitor_steal) / 100;
				if (steal_amt == 0) steal_amt = 1;
				combat_text->addString(msg->get("+%d Capacitor",steal_amt), h.src_stats->pos, COMBAT_MESSAGE_BUFF);
				h.src_stats->capacitor = std::min(h.src_stats->capacitor + steal_amt, h.src_stats->get(STAT_CAPACITOR_MAX));
			}
		}
		if (!stats.effects.immunity_psi_steal && h.psi_steal != 0) {
			if (h.psi_steal != 0) {
				int steal_amt = (std::min(dmg, prev_psi) * h.psi_steal) / 100;
				if (steal_amt == 0) steal_amt = 1;
				combat_text->addString(msg->get("+%d PSI",steal_amt), h.src_stats->pos, COMBAT_MESSAGE_BUFF);
				h.src_stats->psi = std::min(h.src_stats->psi + steal_amt, h.src_stats->get(STAT_PSI_MAX));
			}
		}
