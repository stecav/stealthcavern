...
bool EventManager::loadEventComponentString(std::string &key, std::string &val, Event* evnt, Event_Component* ec) {
	...
	// TODO should intermap and intramap be combined?
	...
	else if (key == "requires_generation") {
		// @ATTR event.requires_generation|int|Event requires hero generation
		e->type = EC_REQUIRES_GENERATION;

		e->x = popFirstInt(val);
	}
	else if (key == "requires_not_generation") {
		// @ATTR event.requires_not_generation|int|Event requires not hero generation
		e->type = EC_REQUIRES_NOT_GENERATION;
	...
	else if (key == "requires_talent") {
		// @ATTR event.requires_talent|predefined_string|Event requires this talent
		e->type = EC_REQUIRES_TALENT;
		...
		// @ATTR event.reward_soul|int|Reward hero with specified amount of soul.
		e->type = EC_REWARD_SOUL;
	...
	else if (key == "gamble_currency") {
		// @ATTR event.reward_currency|int|Gamble specified amount of currency.
		e->type = EC_GAMBLE_CURRENCY;
	...
	else if (key == "reward_stat") {
		// @ATTR event.reward_stat|int|Reward hero with specified stat.
		e->type = EC_REWARD_STAT;

		e->s = popFirstString(val);
		e->x = std::max(toInt(val), 0);
	}
	else if (key == "reward_talent") {
		// @ATTR event.reward_talent|int|Reward hero with specified talent.
		e->type = EC_REWARD_TALENT;

		e->s = popFirstString(val);
		e->x = std::max(toInt(val), 0);
	}
	else if (key == "challenge_stat") {
		// @ATTR event.challenge_stat|int|Challenges npc's stat against hero.
		e->type = EC_CHALLENGE_STAT;
		...
		// @ATTR event.trove|bool|If true, the Trove menu if opened.
		e->type = EC_TROVE;

		e->x = static_cast<int>(toBool(val));
	}
	else if (key == "kiln") {
		// @ATTR event.kiln|bool|If true, the Kiln menu if opened.
		e->type = EC_KILN;
	...
	for (unsigned i = 0; i < ev.components.size(); ++i) {
		...
		if (ec->type == EC_SET_STATUS) {
			...
			if(ec->s == PRIMARY_STATS[y].id){
				if(pc->stats.primary[y] > 5){ 
					pc->logMsg(msg->get("Your %s overpowers their psyche!", PRIMARY_STATS[y].id), false);
				}
				else if (camp->checkCurrency(ec->x)) {		camp->removeCurrency(ec->x); }
			}
		}
		}
		...
		else if (ec->type == EC_REWARD_SOUL) {
			camp->rewardSoul(ec->x, true);
		}
		...
		else if (ec->type == EC_REWARD_STAT) {
			camp->rewardStat(ec->s, ec->x);
		}
		else if (ec->type == EC_REWARD_TALENT) {
			camp->rewardTalent(ec->s, ec->x);
		}
		else if (ec->type == EC_RESTORE) {
			camp->restoreHullCapacitor(ec->s);
		}
		...
		else if (ec->type == EC_TROVE) {
			mapr->trove = static_cast<bool>(ec->x);
			if (mapr->trove) {
				mapr->trove_pos.x = static_cast<float>(ev.location.x) + 0.5f;
				mapr->trove_pos.y = static_cast<float>(ev.location.y) + 0.5f;
			}
		}
		else if (ec->type == EC_KILN) {
			mapr->kiln = static_cast<bool>(ec->x);
			if (mapr->kiln) {
				mapr->kiln_pos.x = static_cast<float>(ev.location.x) + 0.5f;
				mapr->kiln_pos.y = static_cast<float>(ev.location.y) + 0.5f;
			}
		}
