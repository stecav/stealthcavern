...
void PowerManager::loadPowers() {
	...
	while (infile.next()) {
		...
		else if (infile.key == "requires_capacitor")
			// @ATTR requires_capacitor|integer|Restrict power usage to a specified capacitor level.
			powers[input_id].requires_capacitor = toInt(infile.val);
		else if (infile.key == "requires_psi")
			// @ATTR requires_psi|integer|Restrict power usage to a specified PSI level.
			powers[input_id].requires_psi = toInt(infile.val);
		else if (infile.key == "requires_hull")
			// @ATTR power.requires_hull|int|Restrict power usage to a specified HP level.
			powers[input_id].requires_hull = toInt(infile.val);
		else if (infile.key == "requires_limitbreak")
			// @ATTR requires_limitbreak|integer|Restrict power usage to a specified limitbreak level.
			powers[input_id].requires_limitbreak = toInt(infile.val);
		else if (infile.key == "sacrifice")
			// @ATTR power.sacrifice|bool|If the power has requires_hull, allow it to kill the caster.
		...
		//steal effects
		else if (infile.key == "hull_steal")
			// @ATTR power.hull_steal|int|Percentage of damage to steal into HP
			powers[input_id].hull_steal = toInt(infile.val);
		else if (infile.key == "capacitor_steal")
			// @ATTR capacitor_steal|integer|Percentage of damage to steal into Capacitor
			powers[input_id].capacitor_steal = toInt(infile.val);
		else if (infile.key == "psi_steal")
			// @ATTR psi_steal|integer|Percentage of damage to steal into PSI
			powers[input_id].psi_steal = toInt(infile.val);
		//missile modifiers
		...
		// spawn info
			...
			// @ATTR power.spawn_limit|["fixed", "stat", "unlimited"], [int, predefined_string] : Mode, Value|The maximum number of creatures that can be spawned and alive from this power. "fixed" takes an integer. "stat" takes a primary stat as a string (e.g. "brawn").
			...
			// @ATTR power.spawn_limit|["default", "fixed", "stat", "level"], [int, predefined_string] : Mode, Value|The level of spawned creatures. "fixed" and "level" take an integer. "stat" takes a primary stat as a string (e.g. "brawn").
			std::string mode = popFirstString(infile.val);
...
void PowerManager::initHazard(int power_index, StatBlock *src_stats, const FPoint& target, Hazard *haz) {
	...
	// steal effects
	haz->hull_steal += powers[power_index].hull_steal;
	haz->capacitor_steal += powers[power_index].capacitor_steal;
	haz->psi_steal += powers[power_index].psi_steal;
	...
	for (unsigned i=0; i<powers[power_index].post_effects.size(); i++) {
		...
		if (effect_ptr != NULL) {
			...
			if (effect_data.type == "shield") {
				...
				src_stats->hull += magnitude;
				if (src_stats->hull > src_stats->get(STAT_HULL_MAX)) src_stats->hull = src_stats->get(STAT_HULL_MAX);
			}
			else if (effect_data.type == "replentish") {
				// heal for ment weapon damage * damage multiplier
				...
				comb->addString(msg->get("+%d Hull",magnitude), src_stats->pos, COMBAT_MESSAGE_BUFF);
				src_stats->hull += magnitude;
				if (src_stats->hull > src_stats->get(STAT_HULL_MAX)) src_stats->hull = src_stats->get(STAT_HULL_MAX);
				src_stats->psi += magnitude;
				if (src_stats->psi > src_stats->get(STAT_PSI_MAX)) src_stats->psi = src_stats->get(STAT_PSI_MAX);
			}
			else if (effect_data.type == "psi_hull") {
				// heal for ment weapon damage * damage multiplier
				magnitude = randBetween(caster_stats->get(STAT_DMG_MENT_MIN), caster_stats->get(STAT_DMG_MENT_MAX));
				...
				if (src_stats->hull > src_stats->get(STAT_HULL_MAX)) src_stats->hull = src_stats->get(STAT_HULL_MAX);
				src_stats->psi -= magnitude;
				if (src_stats->psi < 0) src_stats->psi = 0;
			}
			else if (effect_data.type == "hull_capacitor") {
				// heal for ment weapon damage * damage multiplier
				magnitude = randBetween(caster_stats->get(STAT_DMG_MENT_MIN), caster_stats->get(STAT_DMG_MENT_MAX));
				...
				comb->addString(msg->get("+%d Hull",magnitude), src_stats->pos, COMBAT_MESSAGE_BUFF);
				if(src_stats->hull > magnitude){
					src_stats->hull -= magnitude;
					if (src_stats->hull > src_stats->get(STAT_HULL_MAX)) src_stats->hull = src_stats->get(STAT_HULL_MAX);
					src_stats->capacitor += magnitude;
					if (src_stats->capacitor > src_stats->get(STAT_CAPACITOR_MAX)) src_stats->capacitor = src_stats->get(STAT_CAPACITOR_MAX);
				}
			}
...
bool PowerManager::activate(int power_index, StatBlock *src_stats, const FPoint& target) {
	...
	if (src_stats->hero) {
		if (powers[power_index].requires_capacitor > src_stats->capacitor && powers[power_index].requires_psi > src_stats->psi)
			return false;
	}
	if (powers[power_index].requires_limitbreak > src_stats->limitbreak) 
		return false;

	if (src_stats->hull > 0 && powers[power_index].sacrifice == false && powers[power_index].requires_hull >= src_stats->hull)
		return false;

	if (power_index ==  151) {
		src_stats->hull = 0;
		src_stats->effects.triggered_death = true;
		src_stats->cur_state = 4;
		return false;
	}
...
void PowerManager::payPowerCost(int power_index, StatBlock *src_stats) {
	if (src_stats) {
		if (src_stats->hero) {
			src_stats->capacitor -= powers[power_index].requires_capacitor;
			src_stats->psi -= powers[power_index].requires_psi;
			if(powers[power_index].requires_limitbreak)
				src_stats->limitbreak = 0;
		...
		src_stats->hull -= powers[power_index].requires_hull;
		src_stats->hull = (src_stats->hull < 0 ? 0 : src_stats->hull);
	}
}
	...
	for (unsigned i=0; i<src_stats->powers_passive.size(); i++) {
		if (powers[src_stats->powers_passive[i]].passive) {
			...
			if (trigger == -1) {
				...
				if (src_stats->hull > src_stats->get(STAT_HULL_MAX)/2) continue;
