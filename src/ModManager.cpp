...
void ModManager::loadModList() {
	...
	if (!found_any_mod && mod_list.size() == 1) {
		logError("ModManager: Couldn't locate any stealthcavern mod. Check if the game data are installed \
                  correctly. Expected to find the data in the $XDG_DATA_DIRS path, in \
				  /usr/local/share/stealthcavern/mods, or in the same folder as the executable. \
...
void ModManager::saveMods() {
	...
	if (outfile.is_open()) {
		// comment
		outfile << "## stealthcavern mods list file ##" << "\n";
