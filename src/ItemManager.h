...
public:
	...
	int base_index; // brawn, conjure, nimble, concentration
	bool is_speed;
...
public:
	...
	int requires_generation;   // Player level must match or exceed this value to use item
	std::vector<size_t> req_stat;        // physical, mental, offense, defense
	std::vector<size_t> req_talent;        // physical, mental, offense, defense
	...
	std::string pickup_status; // when this item is picked up, set a campaign state (usually for mission items)
