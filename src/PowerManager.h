...
public:
	...
	int requires_capacitor;
	int requires_psi;
	int requires_hull;
	int requires_limitbreak;
	bool sacrifice;
	...
	bool no_attack;
	float radius;
	size_t base_damage; // enum.  damage is powered by melee, ranged, mental weapon
	...
	//steal effects (in %, eg. hull_steal=50 turns 50% damage done into HULL regain.)
	int hull_steal;
	int capacitor_steal;
	int psi_steal;
	...
	Power()
		...
		, requires_capacitor(0)
		, requires_psi(0)
		, requires_hull(0)
		, requires_limitbreak(0)
		, sacrifice(false)
		, requires_los(false)
