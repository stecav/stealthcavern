...
void ItemManager::loadItems(const std::string& filename, bool locateFileName) {
	...
	while (infile.next()) {
		...
		if (infile.key == "name") {
			...
			// @ATTR requires_generation|int|The hero's generation must match or exceed this value in order to equip this item.
			items[id].requires_generation = toInt(infile.val);
		}
		else if (infile.key == "requires_stat") {
			// @ATTR requires_stat|repeatable(predefined_string, int) : Primary stat name, Value|Make item require specific stat level ex. requires_stat=brawn,6 will require hero to have level 6 in brawn stat
			if (clear_req_stat) {
			...
			// @ATTR requires_talent|repeatable(predefined_string, int) : talent name, Value|Make item require specific talent level ex. requires_talent=prowess_shields,6 will require hero to have level 6 in prowess_shields
			if (clear_req_talent) {
				items[id].req_talent.clear();
				items[id].req_val.clear();
				clear_req_talent = false;
			}
			std::string s = popFirstString(infile.val);
			size_t req_talent_index = getTalentIndex(s);
			if (req_talent_index != TALENTS.size())
			{
				items[id].req_talent.push_back(req_talent_index);
			}
			else
			{
				infile.error("ItemManager: '%s' is not a valid talent.", s.c_str());
			}
			items[id].req_val.push_back(popFirstInt(infile.val));
		}
			...
			// @ATTR price_per_level|int|Additional price for each player generation above 1
			...
			// @ATTR pickup_status|string|Set a campaign status when item is picked up, this is used for mission items.
		...
		else if (infile.key == "mission_item") {
			// @ATTR mission_item|bool|If true, this item is a mission item and can not be dropped, stashed, or sold.
			items[id].mission_item = toBool(infile.val);
		}
		else {
...
void ItemManager::parseBonus(BonusData& bdata, FileParser& infile) {
	...
	if (bonus_str == PRIMARY_STATS[0].id.c_str()) {
		bdata.base_index = 0;
		return;
	}
	else if (bonus_str == PRIMARY_STATS[1].id.c_str()) {
		bdata.base_index = 1;
		return;
	}
	else if (bonus_str == PRIMARY_STATS[2].id.c_str()) {
		bdata.base_index = 2;
		return;
	}
	else if (bonus_str == PRIMARY_STATS[3].id.c_str()) {
		bdata.base_index = 3;
		return;
	}
	else if (bonus_str == PRIMARY_STATS[4].id.c_str()) {
		bdata.base_index = 5;
		return;
	}
	else if (bonus_str == PRIMARY_STATS[5].id.c_str()) {
		bdata.base_index = 5;
		return;
	}
	else if (bonus_str == PRIMARY_STATS[6].id.c_str()) {
		bdata.base_index = 6;
		return;
	}
	else if (bonus_str == PRIMARY_STATS[7].id.c_str()) {
		bdata.base_index = 7;
		return;
	}
...
TooltipData ItemManager::getTooltip(ItemStack stack, StatBlock *stats, int context) {
	...
	// mission item
	if (items[stack.item].mission_item) {
		tip.addColoredText(msg->get("Mission Item"), color_bonus);
	}
	...
	if (items[stack.item].requires_generation > 0) {
		if (stats->generation < items[stack.item].requires_generation) color = color_requirements_not_met;
		else color = color_normal;
		tip.addColoredText(msg->get("Requires Generation %d", items[stack.item].requires_generation), color);
	}
	...
	// base talent requirement
	for (unsigned i=0; i<items[stack.item].req_talent.size(); ++i) {
		if (items[stack.item].req_val[i] > 0) {
			if (stats->talent[items[stack.item].req_talent[i]] < items[stack.item].req_val[i])
				color = color_requirements_not_met;
			else
				color = color_normal;

			tip.addColoredText(msg->get("Requires %s %d", items[stack.item].req_val[i], TALENTS[items[stack.item].req_talent[i]].name.c_str()), color);
		}
	}
...
bool ItemManager::requirementsMet(const StatBlock *stats, int item) {
	...
	// generation
	if (items[item].requires_generation > 0 && stats->generation < items[item].requires_generation) {
		return false;
	}
	...
	// base talents
	for (unsigned i=0; i < items[item].req_talent.size(); ++i) {
		if (stats->talent[items[item].req_talent[i]] < items[item].req_val[i])
			return false;
	}
...
int Item::getPrice() {
	return price + (price_per_level * (pc->stats.generation - 1));
}
