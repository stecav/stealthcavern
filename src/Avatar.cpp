...
void Avatar::init() {
	...
	current_power = 0;
	newGenerationNotification = false;
...
void Avatar::logic(std::vector<ActionData> &action_queue, bool restrict_power_use, bool npc) {
	...
	if ((stats.hull > 0 || stats.effects.triggered_death) && !respawn && !transform_triggered) powers->activatePassives(&stats);
	...
	// check for revive
	if (stats.hull <= 0 && stats.effects.revive) {
	...
	// check generation up
	if (stats.generation < static_cast<int>(stats.soul_table.size()) && stats.soul >= stats.soul_table[stats.generation]) {
		stats.generation_up = true;
		stats.generation++;
		std::stringstream ss;
		ss << msg->get("Congratulations, you have reached generation %d!", stats.generation);
		if (stats.generation < stats.max_spendable_stat_points) {
			ss << " " << msg->get("You may increase one attribute through the Character Menu.");
			newGenerationNotification = true;
		}
		logMsg(ss.str(), true);
		stats.recalc();
		powers->activate(180, &stats, act_target);
		//setPowers->activate(180,stats,&stats.pos);
		//snd->play(sound_generationup);

		// if the player managed to generation up while dead (e.g. via a bleeding creature), restore to life
	...
	if (stats.hull == 0 && !(stats.cur_state == AVATAR_DEAD)) {
	...
	if (!stats.effects.stun) {
		...
		switch(stats.cur_state) {
			...
			case AVATAR_HIT:
				if (activeAnimation->isFirstFrame()) {
					if(stats.psi > 2) { stats.psi -=2; }//taking hits costs some psi
					else { stats.psi = 0; } //having no psi will cause exhaustion
...
void Avatar::transform() {
	...
	Enemy_Generation el = enemyg->getRandomEnemy(stats.transform_type, 0, 0);
