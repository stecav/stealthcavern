...
void MenuDevConsole::align() {
	...
	label.set(window_area.x+title.x, window_area.y+title.y, title.justify, title.valign, msg->get("DEVELOPER CONSOLE"), font->getColor("menu_header"));
}
...
void MenuDevConsole::execute() {
	...
	if (args[0] == "help") {
		...
		log_history->add("respec - " + msg->get("resets the player to generation 1, with no stat or skill points spent"), false);
