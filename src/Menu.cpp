...
bool Menu::parseMenuKey(const std::string &key, const std::string &val) {
	...
	else if (key == "soundfx_attributeup") {
		// @ATTR soundfx_attributeup|string|Filename of a sound to play when closing this menu.
		sfx_attributeup = snd->load(value, "Character menu attribute button");
	}
	else if (key == "soundfx_unlock") {
		// @ATTR soundfx_unlock|string|Filename of a sound to play when closing this menu.
		sfx_unlock = snd->load(value, "Powers menu button");
	}
	else {
		//not a common key
		return false;
