...
EffectManager::EffectManager()
	...
	, bonus_fealty(std::vector<int>(3, 0))
...
EffectManager& EffectManager::operator= (const EffectManager &emSource) {
	...
	spot = emSource.spot;
	spot_percent = emSource.spot_percent;
	...
	immunity_hull_steal = emSource.immunity_hull_steal;
	immunity_psi_steal = emSource.immunity_psi_steal;
	immunity_capacitor_steal = emSource.immunity_capacitor_steal;
	...
	for (size_t i=0; i<emSource.bonus_fealty.size(); i++) {
		bonus_fealty[i] = emSource.bonus_fealty[i];
	}
...
void EffectManager::clearStatus() {
	...
	immunity_slow = false;
	immunity_stun = false;
	immunity_hull_steal = false;
	immunity_psi_steal = false;
	immunity_capacitor_steal = false;
	...
	for (unsigned i=0; i<bonus_fealty.size(); i++) {
		bonus_fealty[i] = 0;
	}
}
...
void EffectManager::logic() {
	...
	for (unsigned i=0; i<effect_list.size(); i++) {
		...
		if (effect_list[i].duration >= 0) {
			...
			// @TYPE spot|PSI restored per second
			else if (effect_list[i].type == EFFECT_SPOT && effect_list[i].ticks % MAX_FRAMES_PER_SEC == 1) spot += effect_list[i].magnitude;
			// @TYPE spot_percent|PSI restored per second (percentage of max HP)
			else if (effect_list[i].type == EFFECT_SPOT_PERCENT && effect_list[i].ticks % MAX_FRAMES_PER_SEC == 1) spot_percent += effect_list[i].magnitude;
			// @TYPE spot|Capacitor restored per second
			else if (effect_list[i].type == EFFECT_MPOT && effect_list[i].ticks % MAX_FRAMES_PER_SEC == 1) mpot += effect_list[i].magnitude;
			// @TYPE mpot_percent|Capacitor restored per second (percentage of max MP)
			...
			// @TYPE immunity|Applies all immunity effects. Magnitude is ignored.
				...
				immunity_slow = true;
				immunity_stun = true;
				immunity_hull_steal = true;
				immunity_psi_steal = true;
				immunity_capacitor_steal = true;
			...
			// @TYPE immunity_hull_steal|Prevents Hull stealing. Magnitude is ignored.
			else if (effect_list[i].type == EFFECT_IMMUNITY_HULL_STEAL) immunity_hull_steal = true;
			// @TYPE immunity_psi_steal|Prevents PSI stealing. Magnitude is ignored.
			else if (effect_list[i].type == EFFECT_IMMUNITY_PSI_STEAL) immunity_psi_steal = true;
			// @TYPE immunity_capacitor_steal|Prevents Capacitor stealing. Magnitude is ignored.
			else if (effect_list[i].type == EFFECT_IMMUNITY_CAPACITOR_STEAL) immunity_capacitor_steal = true;
			...
			// @TYPE ${PRIMARYSTAT}|Increases ${PRIMARYSTAT}, where ${PRIMARYSTAT} is any of the primary stats defined in engine/primary_stats.txt. Example: physical
				...
				bonus_fealty[effect_list[i].type - EFFECT_COUNT] += effect_list[i].magnitude;
			}
		}
...
int EffectManager::getType(const std::string& type) {
	...
	else if (type == "spot") return EFFECT_SPOT;
	else if (type == "spot_percent") return EFFECT_SPOT_PERCENT;
	...
	else if (type == "immunity_hull_steal") return EFFECT_IMMUNITY_HULL_STEAL;
	else if (type == "immunity_psi_steal") return EFFECT_IMMUNITY_PSI_STEAL;
	else if (type == "immunity_capacitor_steal") return EFFECT_IMMUNITY_CAPACITOR_STEAL;
	...
	else if (type == "fealty") return EFFECT_FEALTY;
	else {
