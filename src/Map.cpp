...
void Map::loadEnemyGroup(FileParser &infile, Map_Group *group) {
	if (infile.key == "type") {
		...
		// @ATTR enemygroup.generation|int, int : Min, Max|Defines the generation range of enemies in group. If only one number is given, it's the exact generation.
		group->generationmin = std::max(0, popFirstInt(infile.val));
		group->generationmax = std::max(0, toInt(popFirstString(infile.val), group->generationmin));
	}
