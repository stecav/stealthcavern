...
HeroClass::HeroClass()
	...
	, equipment("")
	, carried("")
	, primaryStats(PRIMARY_STATS.size(), 0)
	, talents(TALENTS.size(), 0)
...
ConfigEntry config[] = {
	...
	{ "scaling",      &typeid(SCALING),       "1.0", &SCALING,       "scaling for game textures (0.5 = biggest, 1.2 = smallest)"},
	...
	{ "statbar_labels",    &typeid(STATBAR_LABELS),     "0",   &STATBAR_LABELS,     "always show labels on HP/MP/SOUL bars. 1 enable, 0 disable"},
...
int DEATH_PENALTY_SOUL;
int DEATH_PENALTY_SOUL_CURRENT;
...
void loadMiscSettings() {
	...
	DEATH_PENALTY_SOUL = 0;
	DEATH_PENALTY_SOUL_CURRENT = 0;
	...
	if (infile.open("engine/misc.txt")) {
		while (infile.next()) {
			...
			// @ATTR party_exp_percentage|int|The percentage of SOUL given to allies.
	...
	// @CLASS Settings: Talents|Description of engine/talents.txt
	if (infile.open("engine/talents.txt")) {
		while (infile.next()) {
			if (infile.new_section) {
				if (infile.section == "stat") {
					// check if the previous stat is empty and remove it if there is no identifier
					if (!TALENTS.empty() && TALENTS.back().id == "") {
						TALENTS.pop_back();
					}
					TALENTS.resize(TALENTS.size()+1);
				}
			}

			if (TALENTS.empty() || infile.section != "stat")
				continue;

			// @ATTR stat.id|string|An identifier for this talent.
			if (infile.key == "id") TALENTS.back().id = infile.val;
			// @ATTR stat.name|string|The displayed name of this talent.
			else if (infile.key == "name") TALENTS.back().name = msg->get(infile.val);

			else infile.error("Settings: '%s' is not a valid key.", infile.key.c_str());
		}
		infile.close();

		// check if the last talent is empty and remove it if there is no identifier
		if (!TALENTS.empty() && TALENTS.back().id == "") {
			TALENTS.pop_back();
		}
	}
			...
			if (!HERO_CLASSES.empty()) {
				...
				// @ATTR primary|predefined_string, int : Primary stat name, Default value|Class starts with this value for the specified stat.
					...
					if (prim_stat_index != PRIMARY_STATS.size()) {
						HERO_CLASSES.back().primaryStats[prim_stat_index] = toInt(infile.val);
					}
					else {
					...
					std::string seco_stat = popFirstString(infile.val);
					size_t seco_stat_index = getTalentIndex(seco_stat);

					if (seco_stat_index != TALENTS.size()) {
						HERO_CLASSES.back().talents[seco_stat_index] = toInt(infile.val);
					}
					else {
						infile.error("Settings: '%s' is not a valid talent.", seco_stat.c_str());
					}
				}
	...
	if (infile.open("engine/death_penalty.txt")) {
		while (infile.next()) {
			...
			// @ATTR soul_total|int|Remove this percentage of total SOUL.
			else if (infile.key == "soul_total") DEATH_PENALTY_SOUL = toInt(infile.val);
			// @ATTR soul_current_generation|int|Remove this percentage of the Soul gained since the last generation.
			else if (infile.key == "soul_current_generation") DEATH_PENALTY_SOUL_CURRENT = toInt(infile.val);
...
bool saveSettings() {
	...
	if (outfile.is_open()) {
		...
		// comment
		outfile << "## stealthcavern settings file ##" << "\n";
...
size_t getTalentIndex(const std::string& id_str) {
	for (size_t i = 0; i < TALENTS.size(); ++i) {
		if (id_str == TALENTS[i].id)
			return i;
	}
