...
void GameSwitcher::loadBackgroundList() {
	...
	if (infile.open("engine/menu_backimages.txt", true, "")) {
		while (infile.next()) {
			// @ATTR backimage|repeatable(filename)|Filename of a background image to be added to the pool of random menu backgrounds
			if (infile.key == "backimage") background_list.push_back(infile.val);
			else infile.error("GameSwitcher: '%s' is not a valid key.", infile.key.c_str());
		}
		infile.close();
	}
